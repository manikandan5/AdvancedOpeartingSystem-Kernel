...
int fs_close(int filed)
{
    // check if fd is valid
    if ((filed > next_open_fd) || (filed < 0)){
        fprintf(stderr, "fs::fs_close: not valid filed\n\r");
        return SYSERR;
    }
    oft[filed].state = FSTATE_CLOSED;
    return OK;
}
...
int fs_write(int fd, void *buf, int nbytes)
{
    ...
    struct inode in = filetab.in;    
    int inodeoffset=(filetab.fileptr % fsd.blocksz);
    int inodeblk= (filetab.fileptr / fsd.blocksz); 
    int i;
    if(filetab.state==FSTATE_CLOSED){
        ...
        while(nbytes>0){
            ...
            if(nbytes<(fsd.blocksz-inodeoffset)){
                ...
                filetab.fileptr+=nbytes;
                nbytes=0;
                memcpy(oft+fd,&filetab,sizeof(struct filetable));
                return orig_nbytes;
            }
            else{
                if(inodeblk==INODEBLOCKS-1){
                ...
                buf+=(fsd.blocksz-inodeoffset);
                nbytes-=(fsd.blocksz-inodeoffset);
                filetab.fileptr+=(fsd.blocksz-inodeoffset);
                memcpy(oft+fd,&filetab,sizeof(struct filetable));
                inodeblk++;
                inodeoffset=0;
            }
